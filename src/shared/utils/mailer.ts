...
export const sendInvitationEmail = async (email: string, invitationLink: string, expiresAt: string) => {
  try {
    if (!process.env.SMTP_HOST || !process.env.SMTP_USER || !process.env.SMTP_PASS) {
      Logger.error('SMTP configuration is missing', 'shared/utils/mailer.ts - sendInvitationEmail');
      return false;
    }
    ...
    const htmlContent = `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; padding: 40px; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 12px; line-height: 1.6;">
        ...
        <div style="text-align: center; margin-bottom: 30px;">
          <h1 style="color: #f57d28; margin: 0; font-size: 24px;">Provider Invitation</h1>
        </div>
        
        <div style="background-color: #feedd6; border-radius: 8px; padding: 25px; margin-bottom: 30px; border-left: 4px solid #f57d28;">
          <h2 style="margin-top: 0; color: #f57d28; font-size: 20px;">Join Badalin Ecosystem</h2>
          <p style="margin-bottom: 10px;">You have been invited to join the <strong>Badalin Ecosystem</strong> as a Provider.</p>
          <p style="margin: 0; color: #3c5a5e;">This invitation link will expire in <strong>${expiresAt}</strong>.</p>
        </div>

        <p style="margin-bottom: 10px;">Click the button below to complete your registration:</p>
        ...
        <div style="background-color: #f9fafb; padding: 15px; border-radius: 6px; border: 1px solid #e5e7eb; margin-top: 20px;">
          <p style="margin: 0; font-size: 14px; color: #6b7280;"><strong>Note:</strong> If you were not expecting this invitation, please ignore this email.</p>
        </div>
    ...
    const mailOptions = {
      from: `"Badalin | Visa Tracker" <${process.env.SMTP_USER}>`,
      to: email,
      subject: 'Invitation to Join Badalin Ecosystem',
      html: htmlContent,
    };

    await transporter.sendMail(mailOptions);
    Logger.info('Invitation email sent successfully', `Email: ${email}`);
