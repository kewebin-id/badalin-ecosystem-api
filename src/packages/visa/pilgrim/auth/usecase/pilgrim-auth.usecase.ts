    ...
    try {
      ...
      if (user.email) {
        sendAccountActiveEmail(user.email, dto.fullName).catch((err) => {
          Logger.error('Failed to send account activation email', err.message, 'pilgrim-auth.usecase.ts - register');
        });
      }
    ...
    try {
      ...
      if (user.email) {
        ...
        sendResetPasswordEmail(user.email, resetLink).catch((err) => {
          Logger.error('Failed to send reset password email', err.message, 'pilgrim-auth.usecase.ts - forgotPassword');
        });
      }
